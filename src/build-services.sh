...
pushd "$SCRIPTDIR/productpage"
  ...
  #flooding
  docker build --pull -t "${PREFIX}/bookinfo-productpage-v-flooding:${VERSION}" -t "${PREFIX}/bookinfo-productpage-v-flooding:latest" --build-arg flood_factor=100 .
popd
...
pushd "$SCRIPTDIR/details"
  ...
  docker build --pull -t "${PREFIX}/bookinfo-details-v2:${VERSION}" -t "${PREFIX}/bookinfo-details-v2:latest" --build-arg service_version=v2 \
...
pushd "$SCRIPTDIR/ratings"
  docker build --pull -t "${PREFIX}/bookinfo-ratings-v1:${VERSION}" -t "${PREFIX}/bookinfo-ratings-v1:latest" --build-arg service_version=v1 .
  docker build --pull -t "${PREFIX}/bookinfo-ratings-v2:${VERSION}" -t "${PREFIX}/bookinfo-ratings-v2:latest" --build-arg service_version=v2 .
  docker build --pull -t "${PREFIX}/bookinfo-ratings-v-faulty:${VERSION}" -t "${PREFIX}/bookinfo-ratings-v-faulty:latest" --build-arg service_version=v-faulty .
  docker build --pull -t "${PREFIX}/bookinfo-ratings-v-delayed:${VERSION}" -t "${PREFIX}/bookinfo-ratings-v-delayed:latest" --build-arg service_version=v-delayed .
  docker build --pull -t "${PREFIX}/bookinfo-ratings-v-unavailable:${VERSION}" -t "${PREFIX}/bookinfo-ratings-v-unavailable:latest" --build-arg service_version=v-unavailable .
  docker build --pull -t "${PREFIX}/bookinfo-ratings-v-unhealthy:${VERSION}" -t "${PREFIX}/bookinfo-ratings-v-unhealthy:latest" --build-arg service_version=v-unhealthy .
popd

pushd "$SCRIPTDIR/mysql"
  docker build --pull -t "${PREFIX}/bookinfo-mysqldb:${VERSION}" -t "${PREFIX}/bookinfo-mysqldb:latest" .
popd
